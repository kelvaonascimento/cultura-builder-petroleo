...
import { ArrowDownRight, ArrowUpRight, Bot, CheckCircle2, Clock3, FileText, Fuel, Lock, Radar, ShieldCheck, Sparkles, Truck } from "lucide-react";
...
import { CartaoRota, MapaLogistico } from "./demo-map";
...
export function LogisticaView({ sim, tema }: ViewProps) {
  ...
  const sel = sim.rotas.find((r) => r.id === selId) ?? null;
  const emRota = sim.rotas.filter((r) => r.prog < 100);
  return (
    <div className="space-y-3">
      <div className="grid grid-cols-2 gap-3 xl:grid-cols-4">
        <Stat label="Cargas em rota" value={String(emRota.length)} nota="telemetria a cada minuto" />
        <Stat label="Janela cumprida" value={sim.janela + "%"} delta={sim.modo === "integrada" ? "▲ +2,1 p.p." : "▼ -4,2 p.p."} />
        <Stat label="Km vazio" value={fmtDec(sim.kmVazio) + "%"} delta={sim.modo === "integrada" ? "▲ -3,8 p.p." : "▼ +5,1 p.p."} />
        <Stat label="Cargas atrasadas" value={String(emRota.filter((r) => r.atrasada).length)} nota={sim.modo === "integrada" ? "agente já reagendou" : "posto vai ligar perguntando"} />
      </div>

      <div className="relative">
        <MapaLogistico rotas={sim.rotas} modo={sim.modo} tema={tema} onSelect={(id) => setSelId(id || null)} selId={selId} hora={sim.hora} />
        {sel && <CartaoRota rota={sel} tema={tema} onClose={() => setSelId(null)} />}
      </div>

      <div className="grid gap-3 xl:grid-cols-12">
        <PanelCard className="xl:col-span-8" title="Frota ao vivo" sub="Clique numa carga no mapa para o dossiê (lacre, NF-e, desvios)">
          <div className="max-h-64 space-y-1.5 overflow-y-auto pr-1">
            {emRota.map((r) => (
              <button type="button" key={r.id} onClick={() => setSelId(r.id === selId ? null : r.id)} className={`w-full rounded-xl px-3 py-2 text-left transition-colors ${selId === r.id ? "bg-foreground/[0.08] ring-1 ring-foreground/30" : "bg-secondary hover:ring-1 hover:ring-foreground/20"}`}>
                <div className="flex items-center gap-2">
                  <Truck className={`h-3.5 w-3.5 ${r.atrasada ? "text-foreground" : "text-muted-foreground"}`} />
                  <span className="font-mono text-[11px] font-semibold text-foreground">{r.placa}</span>
                  <span className="text-[10.5px] text-muted-foreground">{r.motorista}</span>
                  <span className="h-2 w-2 rounded-full" style={{ background: prodTone(r.produto, tema) }} title={r.produto} />
                  <span className="ml-auto text-[10.5px] tabular-nums text-foreground/70">{Math.round(r.prog)}%</span>
                  {r.atrasada && <Badge variant="outline" className="text-[9px]">atrasada</Badge>}
                  {r.selo ? <Badge variant="secondary" className="text-[9px]">lacre NF-e</Badge> : <Badge variant="outline" className="text-[9px]">lacre manual</Badge>}
                </div>
                <p className="mt-1 text-[10.5px] text-muted-foreground">
                  {r.de} → {r.para} · {r.produto} · {fmtInt(r.volume)} L
                </p>
                <div className="mt-1.5 h-1 w-full overflow-hidden rounded-full bg-foreground/10">
                  <div className="h-full rounded-full bg-foreground transition-[width] duration-700" style={{ width: `${r.prog}%` }} />
                </div>
              </button>
            ))}
          </div>
        ...
        <div className="space-y-3 xl:col-span-4">
          <PanelCard title="Km vazio (14 dias)" sub="Cargas agrupadas pelo agente de rota">
            <div className="h-28">
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={km} margin={{ top: 6, right: 8, bottom: 0, left: -18 }}>
                  ...
                  <Area type="monotone" dataKey="v" stroke={pal.s1} strokeWidth={2} fill="url(#gKm)" />
                </AreaChart>
              </ResponsiveContainer>
            </div>
          </PanelCard>
          <PanelCard title="Janelas de terminal" sub="Slots confirmados pelo agente no portal do terminal">
            <div className="space-y-1.5">
              {[
                ["SFC · Mataripe", "06:00–07:00", "confirmada"],
                ["Pojuca", "07:30–08:00", "confirmada"],
                ["LEM", "09:00–10:00", "aguardando"],
                ["Feira de Santana (hub)", "10:30–11:00", "confirmada"],
              ].map(([b, j, s]) => (
                <div key={b} className="flex items-center justify-between rounded-lg bg-secondary px-3 py-1.5 text-[11px]">
                  <span className="text-foreground/80">{b}</span>
                  <span className="font-mono tabular-nums text-muted-foreground">{j}</span>
