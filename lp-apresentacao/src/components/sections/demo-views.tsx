...
import { Bot, CheckCircle2, Clock3, Fuel, Radar, ShieldCheck, Sparkles } from "lucide-react";
...
import { CockpitView } from "@/demo/cockpit";
import type { ViewId, ViewProps } from "@/demo/telas";
import { AcoesLista, PanelCard, Stat } from "@/demo/ui";

export { VIEW_DEFS, type ViewId, type ViewProps } from "@/demo/telas";
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="grid grid-cols-1 gap-3 @2xl:grid-cols-12">
        ...
        <PanelCard className="@2xl:col-span-4" title="Volume por hora (m³)" sub="Saiu das bases · 14 horas (simulação)">
          <HoraChart tema={tema} />
        ...
        <PanelCard className="@2xl:col-span-3" title="Eventos ao vivo" sub="O que os agentes capturaram agora">
          <FeedEventos sim={sim} max={9} />
        </PanelCard>
      </div>

      <div className="grid grid-cols-1 gap-3 @2xl:grid-cols-12">
        <PanelCard className="@2xl:col-span-7" title="A fila de pedidos atravessa 8 sistemas sem toque humano" sub="No modo manual, cada etapa depende de alguém digitar, ligar ou conferir.">
          <FunilSistemas sim={sim} />
        </PanelCard>
        <PanelCard className="@2xl:col-span-5" title="Margem bruta por produto (R$/m³)" sub="Preço de venda − custo de reposição, média das bases (simulação)">
        ...
        {SISTEMAS.map((s, i) => {
          ...
          return (
            <div key={s} className="relative flex-1">
              ...
              <p className={`mt-1 hidden truncate text-[8.5px] @xl:block ${atual ? "text-foreground" : passou ? "text-muted-foreground" : "text-muted-foreground/40"}`}>{s}</p>
              {atual && <p className="mt-1 text-[8.5px] text-foreground @xl:hidden">em andamento</p>}
            </div>
          );
        })}
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="grid grid-cols-1 gap-3 @2xl:grid-cols-12">
        ...
        <div className="space-y-3 @2xl:col-span-4">
    ...
    <div className="space-y-3">
      ...
      <div className="grid grid-cols-1 gap-3 @2xl:grid-cols-2">
        <PanelCard title="Quanto custa um dia sem reagir" sub={`Impacto na margem do dia, sobre ${fmtInt(VOLUME_DIA / 1000)} m³ vendidos (modelo ilustrativo)`}>
          <div className="space-y-3">
            {sens.map((s) => (
              <div key={s.f} className="grid grid-cols-[minmax(0,1fr)_auto] items-center gap-x-3 gap-y-1 @md:grid-cols-[9.5rem_minmax(0,1fr)_auto]">
                <span className="col-span-2 text-[11px] leading-snug text-foreground/80 @md:col-span-1">{s.f}</span>
...
export function RedeView({ sim, tema, onNav }: ViewProps) {
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="grid grid-cols-1 gap-3 @2xl:grid-cols-12">
        <PanelCard className="@2xl:col-span-7" title="Demanda por posto (m³/dia)" sub="Postos da tabela abaixo · média de 14 dias (simulação)">
          <div className="h-60">
            <ResponsiveContainer width="100%" height="100%">
        ...
        <PanelCard className="@2xl:col-span-5" title="Varejo vigiado por IA" sub="Câmeras + telemetria do posto viram contexto dos agentes" action={<Radar className="h-4 w-4 text-muted-foreground" />}>
          <div className="space-y-2">
            {[
...
export function SuprimentoView({ sim, tema }: ViewProps) {
  ...
  return (
    <div className="space-y-3">
      <div className="grid grid-cols-2 gap-3 @3xl:grid-cols-4">
        <Stat label="Cobertura média" value={fmtDec(sim.tanques.reduce((a, t) => a + t.cover, 0) / sim.tanques.length, 1) + " dias"} nota={sim.modo === "integrada" ? "reposição antecipada pela previsão" : "reposição quando alguém percebe"} />
      ...
      <div className="grid grid-cols-1 gap-3 @2xl:grid-cols-12">
        <PanelCard className="@2xl:col-span-6" title="Tanques por base" sub="Nível e dias de cobertura — simulação">
          <div className="space-y-3">
            {sim.tanques.map((t) => (
              <div key={t.base + t.produto}>
                <div className="mb-1 flex items-center gap-2 text-[11px]">
                  ...
                  <span className="hidden text-muted-foreground @md:inline">· {BASES[t.base]}</span>
                  <span className="text-muted-foreground">· {t.produto}</span>
                  <span className={`ml-auto tabular-nums ${t.nivel < 35 ? "text-foreground font-semibold" : "text-muted-foreground"}`}>{Math.round(t.nivel)}% · {fmtDec(t.cover, 1)} dias</span>
                </div>
        ...
        <div className="space-y-3 @2xl:col-span-6">
...
export function LogisticaView({ sim, tema }: ViewProps) {
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="grid grid-cols-1 gap-3 @2xl:grid-cols-12">
        <PanelCard className="@2xl:col-span-7" title="Km vazio (14 dias)" sub="Cargas agrupadas pelo agente de rota (simulado)">
          <div className="h-40">
            <ResponsiveContainer width="100%" height="100%">
        ...
        <PanelCard className="@2xl:col-span-5" title="Janelas de carregamento" sub="Slots confirmados pelo agente nos terminais (simulado)">
          <div className="space-y-1.5">
            {[
...
export function FinanceiroView({ sim, tema }: ViewProps) {
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="grid grid-cols-1 gap-3 @2xl:grid-cols-12">
        <div className="min-w-0 space-y-3 @2xl:col-span-7">
            ...
            <div className="grid grid-cols-2 gap-2">
              {sim.aging.map((f, i) => (
                <div key={f.faixa} className="rounded-xl bg-secondary p-3">
                  ...
                  <p className={`mt-1 whitespace-nowrap text-[14px] font-semibold tabular-nums @md:text-[15px] ${i === 3 ? "text-destructive" : "text-foreground"}`}>{fmtRs(f.valor)}</p>
                </div>
              ))}
            </div>
  ...
  return (
    <div className="space-y-3">
      ...
      <PanelCard title="Os 8 agentes operando o fluxo" sub="Cada um com dono, foco e trilha de auditoria — sem agente solto na esteira.">
        <div className="grid gap-2.5 @md:grid-cols-2 @4xl:grid-cols-4">
          {sim.agentes.map((g) => (
      ...
      <div className="grid grid-cols-1 gap-3 @2xl:grid-cols-12">
        <PanelCard className="@2xl:col-span-7" title="Dados que estavam dormindo" sub="A IA só entrega quando os dados da operação viram contexto — não relatório.">
          <div className="space-y-1.5">
            {[
        ...
        <PanelCard className="@2xl:col-span-5" title="Aprovação humana onde importa" sub="Agentes propõem; humanos decidem nas exceções.">
          <div className="space-y-1.5">
            {[
...
export function SegurancaView({ sim }: ViewProps) {
  return (
    <div className="space-y-3">
      ...
      <div className="grid grid-cols-1 gap-3 @2xl:grid-cols-12">
        <PanelCard className="@2xl:col-span-6" title="Incidentes e fraudes" sub="Câmeras + telemetria + conciliação de tanque viram contexto dos agentes de compliance">
          <div className="space-y-1.5">
